...
class SystemDiagnostics:
    ...
    def check_python(self):
        ...
        log_success(f"  [OK] Python version: {ver}")
        self.log_report("Python", "Version", "OK", ver)
        
        venv_status = "Active" if in_venv else "Not in venv"
        if in_venv:
            log_success(f"  [OK] Virtual Environment: Active ({sys.prefix})")
        else:
            log_warning("  [WARN] Virtual Environment: Not active")
        ...
        if not missing:
            log_success("  [OK] All required Python packages installed")
            self.log_report("Python", "Packages", "OK", "All packages present")
        else:
            log_error(f"  [FAIL] Missing packages: {', '.join(missing)}")
    ...
    def check_ytdlp(self):
        ...
        if ytdlp["ok"]:
            ...
            log_success(f"  [OK] Installed: {local_ver}")
            ...
            if latest != "Unknown":
                if local_ver != latest:
                    log_warning(f"  [WARN] Update available: {latest} (Current: {local_ver})")
                    self.log_report("yt-dlp", "Updates", "WARNING", f"New version available: {latest}")
                else:
                    log_success("  [OK] Version is up to date")
                    self.log_report("yt-dlp", "Updates", "OK", "Latest version active")
            else:
                log_warning("  [WARN] Could not fetch latest version (Network issue)")
                self.log_report("yt-dlp", "Updates", "UNKNOWN", "Failed to check PyPI version")
            return True
        else:
            log_error("  [FAIL] yt-dlp binary is missing or not in PATH!")
    ...
    def check_ffmpeg(self):
        ...
        if os.getenv("CI") == "true":
            log_success("  [OK] CI environment detected: skipping FFmpeg checks")
    ...
    def check_database(self):
        ...
        if not db_exists:
            log_warning("  [WARN] Database file missing. Attempting auto-rebuild...")
            try:
                database.init_db()
                log_success("  [OK] Database initialized successfully!")
                self.log_report("Database", "File", "REPAIRED", "Recreated successfully")
            except Exception as e:
                log_error(f"  [FAIL] Failed to initialize database: {e}")
                self.log_report("Database", "File", "FAILED", str(e))
                return False
        else:
            log_success(f"  [OK] Database file exists: {db_file}")
        ...
        try:
            ...
            if not missing:
                log_success("  [OK] Database schema verified")
                self.log_report("Database", "Schema", "OK", "All tables present")
            else:
                log_warning(f"  [WARN] Missing tables: {missing}. Repairing schema...")
                database.init_db()
            ...
            if val == '1':
                log_success("  [OK] Database read/write verified")
                self.log_report("Database", "Read/Write", "OK", "Verified")
                return True
            else:
                raise Exception("Write validation failed")
        except Exception as e:
            log_error(f"  [FAIL] Database check failed: {e}. Attempting database repair...")
            ...
            try:
                if os.path.exists(db_file):
                    os.remove(db_file)
                database.init_db()
                log_success("  [OK] Database fully repaired and reinitialized!")
                return True
            except Exception as repair_err:
                log_error(f"  [FAIL] Repair failed: {repair_err}")
                return False
    ...
    def check_storage(self):
        ...
        if not dl_dir.exists():
            log_warning(f"  [WARN] Download folder does not exist. Attempting creation...")
            try:
                dl_dir.mkdir(parents=True, exist_ok=True)
                log_success(f"  [OK] Created: {dl_dir}")
                self.log_report("Storage", "Folder", "REPAIRED", "Created folder")
            except Exception as e:
                log_error(f"  [FAIL] Failed to create download folder: {e}")
                self.log_report("Storage", "Folder", "FAILED", str(e))
                return False
        else:
            log_success(f"  [OK] Download folder exists: {dl_dir}")
        ...
        try:
            with open(test_file, "w") as f:
                f.write("test")
            os.remove(test_file)
            log_success("  [OK] Write permissions: OK")
            self.log_report("Storage", "Permissions", "OK", "Write access verified")
        except Exception as e:
            log_error(f"  [FAIL] Write permissions: FAILED! {e}")
    ...
    def check_network(self):
        ...
        if os.getenv("CI") == "true":
            log_success("  [OK] CI environment detected: skipping network checks")
        ...
        try:
            ...
            log_success("  [OK] Internet connection: Active")
            self.log_report("Network", "Internet", "OK", "Connected")
        except Exception as e:
            log_error("  [FAIL] Internet connection: FAILED! Check connection.")
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=3) as res:
                    ...
                    if code in (200, 301, 302):
                        log_success(f"  [OK] Endpoint {name}: Reachable")
                        self.log_report("Network", name, "OK", "Reachable")
                    else:
                        raise Exception(f"HTTP Status {code}")
            except Exception as e:
                log_warning(f"  [WARN] Endpoint {name}: Failed ({e})")
    ...
    def run_doctor(self):
        ...
        if failed == 0:
            ...
            log_error(f"\n[-] SYSTEM STATUS: {failed} PROBLEMS DETECTED [FAIL]")
